...
## 快速开始（Windows，以下步骤全程实测跑通）

### 第 1 步：安装（一次性，约 5 分钟）

```bat
:: 需 Python 3.11+；py 是 Windows 官方启动器，比 python 命令更稳
py -m venv .venv
...
编辑 `.env`：**本机自用可把 `AUTH_TOKEN` 留空**（免令牌直连）；若 `HOST=0.0.0.0` 部署到局域网/服务器，则必须设一个随机令牌。

### 第 3 步：启动

**双击 `启动火花管家.bat`**——自动启动服务并打开控制台网页（http://127.0.0.1:8020）。保持黑色窗口开着（关窗口 = 停止服务）。

手动方式：`.venv\Scripts\python app.py`（cmd 里先 `chcp 65001` 防中文乱码），然后自己打开浏览器。

### 第 4 步：控制台五步配置（每账号只需一次）

1. **扫码登录**：点「＋添加账号」起个名 → 点「扫码登录」→ 会**弹出一个浏览器窗口**显示二维码（如果先弹出滑块验证，直接拖动滑块通过）→ 用抖音 App「扫一扫」→ 手机上点确认 → 控制台显示「登录成功」
   - 没条件扫码？登录弹窗底部支持粘贴 **Cookie 导入**（F12 → 应用 → Cookie → douyin.com 全选复制）
2. **同步好友**：回到总览点「⟳ 同步好友」，聊天页会话自动进入好友列表（含火花天数）
3. **勾选**：到「好友」页勾选要保火花的人（建议首次只勾 1~2 人观察两天）
4. **模拟演练**：总览点「▶ 模拟演练」——注意演练**只验证流程、不真实发送**
5. **真实发送**：点「⚡ 立即发送」——消息出现在好友聊天窗口才算成功（每条消息都有送达确认，未确认会自动重试）

### 第 5 步：日常运行

- **定时自动**：「定时」页设好每天发送时刻（默认 21:00 整点自动发，默认零抖动到点即发），之后无需任何人工操作
- **推送提醒**：「通知」页配一个渠道——Server酱去 [sct.ftqq.com](https://sct.ftqq.com) 微信扫码免费申请 SendKey 粘贴进去（**注意别把 AUTH_TOKEN 填到这里**），或手机装 Bark 填地址。登录失效、发送失败、触发风控都会推到手机
- **登录过期**：登录态几天到几周会被抖音踢掉一次，收到推送后回控制台重新扫码（10 秒），勾选和配置都还在
- **火花断了别慌**：发送失败会在 23:55 前自动重试；彻底错过会推送告警，次日巡检再提醒
...
cp .env.example .env   # HOST=0.0.0.0，必须设置 AUTH_TOKEN
```

登录说明：控制台点「扫码登录」默认弹**可视浏览器窗口**（需要桌面环境）；
无显示器的服务器会自动退回无头模式并把二维码回传到控制台。
